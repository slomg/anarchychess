...
export function createFakeMovePath(override?: Partial<MovePath>): MovePath {
    return {
        ...
        capturedIdxs: Array(faker.number.int({ min: 1, max: 5 })).map(() =>
            faker.number.int({ min: 0, max: 99 }),
        ),
        triggerIdxs: Array(faker.number.int({ min: 1, max: 5 })).map(() =>
            faker.number.int({ min: 0, max: 99 }),
        ),
        sideEffects: Array(faker.number.int({ min: 1, max: 5 })).map(() =>

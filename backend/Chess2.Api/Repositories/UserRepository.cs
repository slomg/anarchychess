...
public interface IUserRepository
{
    ...
    public Task<User> UpdateUserProfileAsync(User user, UserProfileUpdate userEdit, CancellationToken cancellation = default);
}
...
public class UserRepository(Chess2DbContext dbContext) : IUserRepository
{
    ...
    /// This method takes the properties of <see cref="UserProfileUpdate"/> and
    /// updates them in the user if it's not null
    /// </summary>
    public async Task<User> UpdateUserProfileAsync(User user, UserProfileUpdate userEdit, CancellationToken cancellation = default)
    {

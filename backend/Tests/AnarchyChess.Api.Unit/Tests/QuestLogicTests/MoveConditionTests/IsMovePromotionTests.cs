...
public class IsMovePromotionTests
{
    ...
    [Fact]
    public void Evaluate_returns_true_for_specific_piece_promotion()
    {
        var move = new MoveFaker().RuleFor(x => x.PromotesTo, PieceType.Rook).Generate();
        new IsMovePromotion(PieceType.Rook).Evaluate(move).Should().BeTrue();
    }

    [Fact]
    public void Evaluate_returns_false_for_different_piece_promotion()
    {
        var move = new MoveFaker().RuleFor(x => x.PromotesTo, PieceType.Queen).Generate();
        new IsMovePromotion(PieceType.Rook).Evaluate(move).Should().BeFalse();
    }
}
